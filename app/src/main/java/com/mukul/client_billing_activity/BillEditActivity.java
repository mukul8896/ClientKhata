...
public class BillEditActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            bill=DBServices.getBill(getIntent().getIntExtra("bill_id",0));
            String bill_details=bill.getBill_year()+" | Bill No-"+bill.getBill_no();
            toolbar.setTitle(bill_details);
            transectionList= DBServices.getBillTransection(bill_details);
        } catch (Exception e) {
    ...
    public boolean onContextItemSelected(MenuItem item) {
        ...
        if (id == R.id.delete) {
            try {
                DBServices.deleteTransection(bill.getClient_id(),transectionList.get(index));
